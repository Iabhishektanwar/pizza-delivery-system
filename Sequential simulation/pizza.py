...
class Pizzas:
    ...
    def create_pizza(self, pizza):
        ...
        if pizza not in self.pizza:
            ...
            ingredients_for_pizza = Ingredients().ingredients_available(pizza)
            if ingredients_for_pizza is not None:
                sleep(0.02)
                return ingredients_for_pizza
            return None

    def cook_pizza(self, order, time_of_order):
        ...
        for pizza in order:
            for i in range(0, order[pizza]):
                ingredients_required_for_pizza_making = self.create_pizza(pizza)
        ...
        Orders().collection_queue(processed_order)
        return True
