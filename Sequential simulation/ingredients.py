...
class Ingredients:
    ...
    def get_required_ingredients(self, pizza, toppings):
        ...
        if pizza not in self.pizza_size:
            ...
            ingredient_list = storage.Storage().update_storage(ingredients_required, toppings)
            return ingredient_list

    def ingredients_available(self, pizza):
        """checks if ingredients are available for pizza"""
        if pizza not in self.pizza_size:
            return None
        else:
            available_ingredients = storage.Storage().available_ingredients_in_storge()
            ...
            if pizza == self.pizza_size[0]:
                ...
                if available_ingredients['dough'] >= 3 and available_ingredients['sauce'] >= 2 and \
                    ...
                    ingredients_required_for_pizza = self.get_required_ingredients(pizza, toppings)
